...
def evolve_cluster_in_galaxy(options_file):
    ...
    newpart.x = starpos[0] | units.kpc
    newpart.y = starpos[1] | units.kpc
    newpart.z = starpos[2] | units.kpc
    newpart.vx = starvel[0] | units.kms
    newpart.vy = starvel[1] | units.kms
    newpart.vz = starvel[2] | units.kms

    stars.add_particle(newpart)
